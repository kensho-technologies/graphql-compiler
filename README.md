...
## Getting Started
The first thing you'll need is to build a GraphQL schema. The GraphQL schema is a GraphQL 
...
url = 'memory://{}/{}'.format(ORIENTDB_SERVER, DATABASE_NAME)
...
client.db_open(DATABASE_NAME, ORIENTDB_USER, ORIENTDB_PASSWORD, db_type=DB_TYPE_GRAPH)

# Step 3: Build database schema.
client.command('CREATE CLASS Animal EXTENDS V')
client.command('CREATE PROPERTY Animal.name String')
client.command('CREATE PROPERTY Animal.net_worth Decimal')

# Step 4: Populate database.
client.command('create vertex Animal set name = \'Animal 1\', net_worth = Decimal(\'100\')')
client.command('create vertex Animal set name = \'Animal 2\', net_worth = Decimal(\'200\')')

# Step 5: Generate GraphQL schema from the database schema metadata.
...
# Step 6: Write GraphQL query to get the names of all animals with a particular net worth.
...
# Step 7: Use autogenerated GraphQL schema to compile query into the target database language.
compilation_result = graphql_to_match(schema, graphql_query, parameters, type_equivalence_hints)

# Step 8: Run compiled query.
