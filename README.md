...
about the database without executing the query.
...
query is being executed on, the current version of the cost estimator estimates the query's
cardinality (i.e. the number of rows in the result) as a rough proxy for time/memory usage.
...
# edges, and FeedingEvent vertices.
class_counts = {
    'Animal': 6,
...
# This dictionary describes the number of Animal_ImportantEvent edges specifically
# connecting Animal and FeedingEvent vertices.
vertex_edge_vertex_counts = {
    ('Animal', 'Animal_ImportantEvent', 'FeedingEvent'): 10,
}
...
- `LocalStatistics`, an implementation of `Statistics`, where the statistics are stored in-memory
  and are provided during initialization, and can't be modified thereafter.

The list of supported statistics can be found in `graphql_compiler/cost_estimation/statistics.py`,
