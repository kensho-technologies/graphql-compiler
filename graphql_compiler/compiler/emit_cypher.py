...
def _emit_code_from_cypher_step(cypher_step):
    ...
    if no_linked_location ^ step_is_query_root:
        raise AssertionError(
            u"Received an illegal CypherStep. Not having a linked location is "
            u"allowed if and only if the step is with a QueryRoot object. {}".format(cypher_step)
        )
    ...
    if has_where_block and is_optional_step:
        raise AssertionError(
            u"Received an illegal CypherStep containing an optional step together "
...
def _emit_with_clause_components_for_current_fold_scope(current_fold_scope_cypher_steps):
    ...
    for cypher_step in current_fold_scope_cypher_steps:
        if not isinstance(cypher_step, CypherStep):
            raise TypeError(
                u"Expected current_fold_scope_cypher_steps to contain only CypherStep "
                u"objects. Instead, got object {} of type {}. "
                u"current_fold_scope_cypher_steps: {}".format(
