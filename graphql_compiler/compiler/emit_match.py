...
def _get_vertex_location_name(location):
    ...
    if field_name is not None:
        raise AssertionError("Location unexpectedly pointed to a field: {}".format(location))
    ...
    if match_step.root_block is not None:
        if not isinstance(match_step.root_block, QueryRoot):
            raise AssertionError(
                "Expected None or QueryRoot root block, received: "
                "{} {}".format(match_step.root_block, match_step)
            )

        match_step.root_block.validate()

        start_class = get_only_element_from_collection(match_step.root_block.start_class)
        parts.append("class: %s" % (start_class,))

    # MATCH steps with a QueryRoot root block shouldn't have a 'coerce_type_block'.
    if match_step.coerce_type_block is not None:
        raise AssertionError("Invalid MATCH step: {}".format(match_step))

    if match_step.where_block:
        match_step.where_block.validate()
        parts.append("where: (%s)" % (match_step.where_block.predicate.to_match(),))

    if match_step.as_block is None:
        raise AssertionError(
            "Found a MATCH step without a corresponding Location. "
            "This should never happen: {}".format(match_step)
        )
    else:
        match_step.as_block.validate()
        parts.append("as: %s" % (_get_vertex_location_name(match_step.as_block.location),))
...
def _subsequent_step_to_match(match_step):
    ...
    if not isinstance(match_step.root_block, (Traverse, Recurse)):
        raise AssertionError(
            "Expected Traverse root block, received: "
            "{} {}".format(match_step.root_block, match_step)
        )
    ...
    traversal_command = ".%s('%s')" % (
        match_step.root_block.direction,
    ...
    if match_step.coerce_type_block:
        ...
        if len(coerce_type_set) != 1:
            raise AssertionError(
                "Found MATCH type coercion block with more than one target class:"
                " {} {}".format(coerce_type_set, match_step)
            )
        coerce_type_target = list(coerce_type_set)[0]
        parts.append("class: %s" % (coerce_type_target,))
    ...
    if is_recursing:
        ...
        parts.append("while: ($depth < %d)" % (match_step.root_block.depth,))
    ...
    return "%s {{ %s }}" % (traversal_command, ", ".join(parts))
...
def _represent_fold(fold_location, fold_ir_blocks):
    ...
    for block in fold_ir_blocks:
        if isinstance(block, Filter):
            final_string += "[" + block.predicate.to_match() + "]"
            ...
            raise AssertionError(
                ...
                "{} {} {}".format(type(block), block, fold_ir_blocks)
            )
...
def _construct_where_to_match(where_block):
    ...
    if where_block.predicate == TrueLiteral:
        raise AssertionError(
            "Received WHERE block with TrueLiteral predicate: {}".format(where_block)
        )
    return "WHERE " + where_block.predicate.to_match()
...
def emit_code_from_single_match_query(match_query):
    ...
    query_data = deque(["MATCH "])

    if not match_query.match_traversals:
        raise AssertionError(
            "Unexpected falsy value for match_query.match_traversals received: "
            "{} {}".format(match_query.match_traversals, match_query)
        )
    ...
    query_data.appendleft(" (")  # Prepare to wrap the MATCH in a SELECT.
    query_data.append("RETURN $matches)")  # Finish the MATCH query and the wrapping ().
...
def emit_code_from_multiple_match_queries(match_queries):
    ...
    query_data = deque(["SELECT EXPAND(", union_variable_name, ")", " LET "])
    ...
    query_data.append(" = UNIONALL(")
    query_data.append(", ".join(optional_variables))
    query_data.append(")")
...
def emit_code_from_ir(schema_info, compound_match_query):
    ...
    if len(match_queries) == 1:
        ...
        raise AssertionError(
            "Received CompoundMatchQuery with an empty list of MatchQueries: "
            "{}".format(match_queries)
        )
